...
class TopicWriter:
    ...
    def getRawCorpus(self, csv_file):
        raw_corpus = []
        reader = csv.reader(csv_file, delimiter='|', quotechar='"')
        i = 0
        for row in reader:
    ...
    def do(self,originfile):
        ...
        for emotion in ['Good', 'Bad']:
            ...
            for keyword in keywords.keys():
                print(keyword)
                raw_corpus=self.getRawCorpus(csv_file = open('resources/csvs/' + keyword + '_' + emotion.lower() + '.csv', mode='r',
                ...
                nationalitiescount={}
                nationalitiescount =self.getNationsCountDict(csv_file = open('resources/csvs/' + keyword + '_' + emotion.lower() + '.csv', mode='r',
                                encoding="utf8", newline='\n'))
                # Create stopword list:
                stopwords=self.getStopwords(stopset)
                ...
                # the matplotlib way:
                self.display_save_wordcloud(wordcloud,display=False,save=True,path='resources/' + keyword + '_' + emotion.lower() + 'wordcloud.png')

                '''
                #https://spacy.io/usage/processing-pipelines
                nlp = spacy.load('en', disable=['parser', 'ner'])
                data_lemmatized = self.lemmatization(list_of_list_of_tokens, nlp,allowed_postags=['NOUN', 'ADJ', 'VERB', 'ADV'])
                ...
                topic_keywords = self.show_lda_topics(lda_model=best_lda_model, n_words=15,df_topic_keywords=df_topic_keywords)
